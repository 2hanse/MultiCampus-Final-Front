...
const JoinPage = () => {
  ...
  const checkNickName = useCallback(
    (e) => {
      e.preventDefault();
      api
        .post('/users/nickname-exists', nickName, {
          headers: {
            'Content-Type': 'text/plain',
          },
        })
  ...
  const handleVerificationRequest = async () => {
    try {
      // 중복 번호 확인
      await api.post(`/users/phonenumber-exists`, phoneNum, {
        headers: {
          'Content-Type': 'text/plain',
        },
      });
      ...
      // 서버에 인증번호 발송 요청
      await api.post('/sms/send', phoneNum, {
        headers: {
          'Content-Type': 'text/plain',
        },
      });
      ...
      // 성공 시 처리
      setIsVerificationSent(true); // 인증번호 발송 성공 시 상태를 true로 설정
      setPhoneNumMessage('');
    } catch (error) {
      // 에러 처리
    ...
    <StyledForm onSubmit={onSubmit}>
      ...
      <main>
        ...
        <StyledInputField>
          ...
          <StyledButton small onClick={checkEmail}>
            중복확인
          </StyledButton>
        </StyledInputField>
        <Formbox>
        ...
        <StyledInputField>
          ...
          <StyledButton small onClick={checkNickName} type="button">
            중복확인
          </StyledButton>
        </StyledInputField>
        <Formbox>
